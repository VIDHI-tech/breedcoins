...
export const StudentCouncilGvTable = () => {
  const { data } = useApiQuery(endpoints.entities.table_student_council_gv.all);
  const tableFilteredData = useMemo(() => data?.data?.data ?? [], [data]);
  // console.log("studentcouncilgv", tableFilteredData);
  ...
  const columns = [
    ...
    { key: "commitee_member_name", header: "Staff Members" },
